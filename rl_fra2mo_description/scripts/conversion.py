...
from tf_transformations import quaternion_from_euler, quaternion_from_matrix, euler_from_quaternion
...
def main():
    ...
    def create_pose(transform):
        ...
        composed_matrix = tf_transformations.concatenate_matrices(t2_matrix,t1_matrix)
    ...
    if strategy == "aruco":
        ...
        for pose, name  in goals:
            if name == target_nameex:
                goal_poses.append(pose)
                print(f"pose_traslation: {pose.pose.position}")
                rpy=tf_transformations.euler_from_quaternion([pose.pose.orientation.x,pose.pose.orientation.y,pose.pose.orientation.z,pose.pose.orientation.w])
                print(f"pose_orienation: {rpy}")
